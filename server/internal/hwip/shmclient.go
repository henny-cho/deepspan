...
// ShmClient provides synchronous command submission to hw-model via POSIX shm.
...
// ShmClient is safe for concurrent use; internal mutex serialises commands
// since the hw-model RegMap has no per-slot command queuing.
type ShmClient struct {
	mu      sync.Mutex
	shmPath string // empty → stub mode (no real shm)
}

func newShmClient(shmName string) *ShmClient {
	path := ""
	if shmName != "" {
		path = "/dev/shm/" + shmName
	}
	return &ShmClient{shmPath: path}
}

// SubmitCmd writes a command to the hw-model RegMap and returns the result.
// If shmPath is empty (sim/test mode) it returns a stub response.
func (c *ShmClient) SubmitCmd(opcode, arg0, arg1 uint32, timeoutMs uint32) (status, data0, data1 uint32, err error) {
	...
	for atomic.LoadUint32(ctrlPtr)&ctrlStart != 0 {
		if time.Now().After(deadline) {
			return 0, 0, 0, fmt.Errorf("timeout (%dms) waiting for hw-model", timeoutMs)
		}
