...
```bash
./scripts/dev.sh build                           # dev 프리셋 (기본)
./scripts/dev.sh build --preset dev-hwip         # HWIP 플러그인 포함
...
프리셋은 `CMakePresets.json` 참조: `dev`, `dev-submodule`, `sim`, `release`, `coverage`, `dev-hwip`, `dev-multi-hwip`, `dev-crc32`, `arm64-cross`.
...
```bash
./scripts/dev.sh test                    # dev-hwip 프리셋 빌드 + 시뮬레이션 실행
