...
/*static*/
tl::expected<std::unique_ptr<DevicePool>, deepspan::userlib::Error>
DevicePool::create(std::vector<std::string> device_paths, unsigned uring_queue_depth) {
    ...
    for (const auto& path : device_paths) {
        ...
        if (!dev_result.has_value()) {
            return tl::make_unexpected(dev_result.error());
        }
        ...
        if (!client_result.has_value()) {
            return tl::make_unexpected(client_result.error());
        }
...
tl::expected<DevicePool::Guard, deepspan::userlib::Error> DevicePool::acquire() {
    ...
    return tl::make_unexpected(deepspan::userlib::Error::DeviceOpenFailed);
}
